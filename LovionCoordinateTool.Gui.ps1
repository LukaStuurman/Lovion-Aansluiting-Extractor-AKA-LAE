...
function Get-LovionScreenCounters {
    ...
    try {
        ...
        foreach ($item in @(
            ...
            $match = [regex]::Match($ocrText, ('(?i){0}\s*[:;]?\s*([0-9IOil|]+)' -f $item.Pattern))
            if ($match.Success) {
...
function Set-LovionSelectionDelta {
    ...
    for ($index = 0; $index -lt [Math]::Abs($Delta); $index += 1) {
        [LovionBatchInput]::KeyDown(0x10)
        try {
            Start-Sleep -Milliseconds 35
            Invoke-LovionKeyTap -VirtualKey $key -DelayMs $KeyDelayMs
        }
        finally {
            [LovionBatchInput]::KeyUp(0x10)
        }
    }
...
function Move-LovionToNextStartRow {
    ...
    Invoke-LovionKeyTap -VirtualKey 0x28 -DelayMs 160 -FastMouseMode:$FastMouseMode
    [LovionBatchInput]::ReleaseModifiers()
...
function Get-NormalizedLovionSelection {
    ...
    if ($TargetCount -gt 1) {
        $rowsToScroll = [Math]::Max(0, ($TargetCount - 1) - ($lastVisibleRowIndex - $activeRowIndex))
        $wheelSteps = [int][Math]::Ceiling($rowsToScroll / [double]$rowsPerWheelStep)
        Set-LovionBatchStatus ("Lovion selectie: {0} rijen via scrollen" -f $TargetCount)
        if ($wheelSteps -gt 0) {
            if (-not (Invoke-LovionGridScroll -WheelSteps $wheelSteps -FastMouseMode:$FastMouseMode)) {
                throw 'De Lovion-tabel kon niet automatisch worden gescrold.'
            }
            $netWheelSteps += $wheelSteps
            Write-LovionBatchLog ("Selectiescroll: {0} wielstappen omlaag" -f $wheelSteps)
            Start-Sleep -Milliseconds ([Math]::Min(1800, 500 + ($wheelSteps * 35)))
        }
        if (-not (Invoke-LovionShiftClickRow -VisibleRowIndex $lastVisibleRowIndex -FastMouseMode:$FastMouseMode)) {
            throw 'De Lovion-selectie kon niet met Shift+klik worden uitgebreid.'
        }
        $activeRowIndex = $lastVisibleRowIndex
        Start-Sleep -Milliseconds 450
    }
    ...
    for ($attempt = 0; $attempt -le $MaxCorrections -and $selectedCount -ne $TargetCount; $attempt += 1) {
        ...
        Write-LovionBatchLog ("Scrollselectie OCR {0}: geselecteerd={1}, doel={2}" -f ($attempt + 1), $selectedCount, $TargetCount)
        ...
        if ($attempt -eq 0 -and $TargetCount -gt 1 -and $selectedCount -le 1) {
            Set-LovionBatchStatus 'Lovion heeft de eerste Shift+klik gemist; selectie opnieuw opbouwen'
            if ($netWheelSteps -ne 0) {
                if (-not (Invoke-LovionGridScroll -WheelSteps (-$netWheelSteps) -FastMouseMode:$FastMouseMode)) {
                    throw 'De Lovion-tabel kon niet terugscrollen om de selectie opnieuw op te bouwen.'
                }
                Start-Sleep -Milliseconds ([Math]::Min(1800, 500 + ([Math]::Abs($netWheelSteps) * 35)))
            }
            if (-not (Invoke-LovionClickRow -VisibleRowIndex $StartRowIndex -FastMouseMode:$FastMouseMode)) {
                throw 'De eerste Lovion-rij kon niet opnieuw als selectieanker worden gekozen.'
            }
            if ($netWheelSteps -ne 0) {
                if (-not (Invoke-LovionGridScroll -WheelSteps $netWheelSteps -FastMouseMode:$FastMouseMode)) {
                    throw 'De Lovion-tabel kon de selectiepositie niet opnieuw bereiken.'
                }
                Start-Sleep -Milliseconds ([Math]::Min(1800, 500 + ([Math]::Abs($netWheelSteps) * 35)))
            }
            if (-not (Invoke-LovionShiftClickRow -VisibleRowIndex $lastVisibleRowIndex -FastMouseMode:$FastMouseMode)) {
                throw 'De Lovion-selectie kon niet opnieuw met Shift+klik worden opgebouwd.'
            }
            $activeRowIndex = $lastVisibleRowIndex
            Write-LovionBatchLog 'Eerste Shift+klik gemist; selectieanker en scrollpositie opnieuw opgebouwd.'
        ...
        $candidateRowIndex = $activeRowIndex + $delta
        Set-LovionBatchStatus ("Lovion scrollselectie corrigeren: {0} -> {1}" -f $selectedCount, $TargetCount)
        if ($candidateRowIndex -ge 0 -and $candidateRowIndex -le $lastVisibleRowIndex) {
            if (-not (Invoke-LovionShiftClickRow -VisibleRowIndex ([int]$candidateRowIndex) -FastMouseMode:$FastMouseMode)) {
                throw 'De Lovion-selectie kon niet met Shift+klik worden gecorrigeerd.'
            }
            $activeRowIndex = [int]$candidateRowIndex
            Write-LovionBatchLog ("Correctie met Shift+klik op zichtbare rij {0}" -f ($activeRowIndex + 1))
        }
        else {
            $correctionSteps = [int][Math]::Max(1, [Math]::Ceiling([Math]::Abs($delta) / [double]$rowsPerWheelStep))
            $signedSteps = if ($delta -gt 0) { $correctionSteps } else { -$correctionSteps }
            if (-not (Invoke-LovionGridScroll -WheelSteps $signedSteps -FastMouseMode:$FastMouseMode)) {
                throw 'De Lovion-tabel kon niet worden gescrold tijdens de selectiecorrectie.'
            }
            $netWheelSteps += $signedSteps
            Start-Sleep -Milliseconds ([Math]::Min(1200, 350 + ($correctionSteps * 35)))
            if (-not (Invoke-LovionShiftClickRow -VisibleRowIndex $lastVisibleRowIndex -FastMouseMode:$FastMouseMode)) {
                throw 'De Lovion-selectie kon na het scrollen niet worden gecorrigeerd.'
            }
            $activeRowIndex = $lastVisibleRowIndex
            Write-LovionBatchLog ("Correctiescroll: {0} wielstappen" -f $signedSteps)
        }
        Start-Sleep -Milliseconds 400
    }
    ...
    $content = Copy-LovionSelectionText -CopyWaitMs $CopyWaitMs -FastMouseMode:$FastMouseMode
    Assert-LovionAutomationNotCancelled
...
function Open-LovionConnectionsForStation {
    ...
    if ($null -eq (Wait-LovionScreenPattern -Pattern 'LS\s+Stroomtransformatorgroep' -TimeoutSeconds 8 -Description 'de startlijst LS Stroomtransformatorgroep')) {
    ...
    Invoke-LovionStationClick -X 70 -Y 47 -WaitMilliseconds 450 -Description 'de tab EXPLORE'
    Invoke-LovionStationClick -X 78 -Y 105 -WaitMilliseconds 1400 -Description 'Start query'
    Invoke-LovionStationClick -X 136 -Y 47 -WaitMilliseconds 450 -Description 'de tab Open'
    Invoke-LovionStationClick -X 70 -Y 92 -WaitMilliseconds 3200 -Description 'VIEW'

    Set-LovionBatchStatus ("Station {0}/{1}: netwerkweergave openen" -f $StationIndex, $StationCount)
    Invoke-LovionStationClick -X 279 -Y 49 -WaitMilliseconds 3000 -Description 'de netwerktab'
    Invoke-LovionStationClick -X 387 -Y 99 -WaitMilliseconds 450 -Description 'Schakelaar instellen'
    Invoke-LovionStationClick -X 440 -Y 222 -WaitMilliseconds 850 -Description 'de schakelaaroptie'
    Invoke-LovionStationClick -X 790 -Y 93 -WaitMilliseconds 450 -Description 'Aansluitingen instellen'
    Invoke-LovionStationClick -X 754 -Y 142 -WaitMilliseconds 850 -Description 'de aansluitingenoptie'
    ...
    Invoke-LovionStationClick -X 957 -Y 590 -WaitMilliseconds 1800 -Description 'het station op de kaart'
    if ($null -eq (Wait-LovionScreenPattern -Pattern 'Terug.?zetten|Reset' -TimeoutSeconds 18 -Description 'Terugzetten na de netwerkselectie')) {
        Start-Sleep -Seconds 2
    }

    Invoke-LovionStationClick -X 465 -Y 89 -WaitMilliseconds 1200 -Description 'het eerste netwerkresultaat'
    if ($null -eq (Wait-LovionScreenPattern -Pattern 'Exporteren' -TimeoutSeconds 18 -Description 'de resultaatlijst')) {
        Start-Sleep -Seconds 2
    }
    Invoke-LovionStationClick -X 901 -Y 102 -WaitMilliseconds 1400 -Description 'de aansluitingenlijst'

    $connections = Wait-LovionConnectionsTable -TimeoutSeconds 25
...
function Invoke-LovionStationsImportCore {
    ...
    for ($index = 0; $index -lt $stationCount; $index += 1) {
        ...
        try {
            Open-LovionConnectionsForStation -StationNumber $station -StationIndex ($index + 1) -StationCount $stationCount
...
function Start-LovionStationsImport {
    ...
    Show-InfoMessage ("Stationsimport klaar.`n`nStations: {0}`nAansluitingen: {1}`n`n{2}" -f $result.StationCount, $result.ImportedCount, $details)
}
